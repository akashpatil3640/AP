    ...
    return new Promise((resolve,reject)=>{
        ...
        saveFile(fileName,data).then(()=>{
            ...
            fs.writeFile("input.txt",input,function(err){
                if(err)
                {
                    console.log("error while creating the input.txt file for java",err);  
                    reject();
                }
                else
                    console.log("Successfully saved the input.txt file for java");
            });
            ...
            exec('javac '+filePath,(err,stdout,stderr)=>{
                ...
                exec('java test < '+'input.txt',(err,stdout,stderr)=>{
                    if(err){
                        console.log("error while executing /test.class file!! ");
                        resolve({
                            err:err,
                            output:err,
                            error:stderr
