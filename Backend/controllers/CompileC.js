    ...
    return new Promise((resolve,reject)=>{
        ...
        saveFile(fileName,data).then(()=>{
            ...
            fs.writeFile("input.txt",input,function(err){
                if(err)
                {
                    console.log("error while creating the input.txt file for c",err);  
                    reject();
                }
                else
                    console.log("Successfully saved the input.txt file for c");
            });
            ...
            exec('gcc '+filePath,(err,stdout,stderr)=>{
                ...
                exec('./a.out< '+'input.txt',(err,stdout,stderr)=>{
                    if(err){
                        console.log("error while executing /a.out file!! ");
                        resolve({
                            err:err,
                            output:err,
                            error:stderr
